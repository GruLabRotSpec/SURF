...
class SpectrometerController(QObject):
    ...
    def run_search(self, freq=9000, step_size=0.5,):
        ...
        self.current_task = asyncio.create_task(self._run_search_async(freq, step_size,signals=ScanSignals))

    async def _run_search_async(self, freq, step_size, signals:ScanSignals):
        try:
            await asyncio.to_thread(self.spectrometer.cavity_search, freq, step_size, signals)
            if self.cancel_event.is_set():
